...
|---|---|---|
| executive-brief | prose | One-page sponsor status, anchored to the outcome |
| risk-intelligence | prose | Ranked, causal, decision-oriented risk radar |
| stakeholder-email | prose | Drafts the PM's update email content (never sends) |
| decision-register | prose | Decisions made + decisions needed (with recommendations) |
| action-register | prose | Consolidated actions; overdue/unowned flagged |
| outcome-dashboard | **code** | Progress vs. the business outcome; computes + charts metrics |
| drive-report-publisher | **write** | Writes a finished report to the Atlas Shared Drive (direct write — internal artefact) |
| slack-notifier | **write** | Posts an update to `#atlas` (drafts for review by default; direct send only on explicit instruction) |
| gmail-stakeholder-update | **write** | Places the stakeholder email into Gmail as a draft (never sends autonomously) |

The first six are reasoning procedures the model follows (**outcome-dashboard** additionally runs `compute_metrics.py` so its numbers are calculated, not estimated). The last three are **publishing** skills that write to the live toolchain, under a governance gradient: internal artefacts (Drive) write directly; anything reaching stakeholders (Slack, Gmail) defaults to a draft for human review.
