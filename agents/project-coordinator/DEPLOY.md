# Deploy Guide — Project Coordinator Agent
...
- **Settings → Capabilities → enable Code execution & File creation** — required for skills to run, and specifically for `outcome-dashboard` to compute/chart. Without it, the dashboard skill falls back to a prose-only view.
- Atlassian connector (Jira + Confluence) connected, with access to project **AT** and space **Atlas**.
- Google Drive + Gmail connectors (write access) for `drive-report-publisher` and `gmail-stakeholder-update`; Slack connector (write access) for `slack-notifier`. All three publishing skills degrade gracefully (report the missing connector) if not connected.
...
```bash
cd skills
for d in executive-brief risk-intelligence stakeholder-email decision-register action-register outcome-dashboard drive-report-publisher slack-notifier gmail-stakeholder-update; do
  (cd "$d" && zip -r "../../dist/$d.zip" . -x '*.DS_Store')
done
```
This produces nine ZIPs in `dist/`. (The `outcome-dashboard.zip` includes its `scripts/` folder.)
...
Verify: start a chat and ask, e.g., *"Use the risk-intelligence skill for Project Atlas."* It should announce `▸ Skill: Risk Intelligence` and run.

## Step 3 — Create the Project
1. New Project → name it e.g. *Project Coordinator — Atlas*.
...
- **No code fallback:** if Code execution is off, `outcome-dashboard` still works as a prose outcome view — it just won't compute/chart.
- **Publishing governance gradient:** `drive-report-publisher` writes directly (internal artefact, the team's own workspace). `slack-notifier` and `gmail-stakeholder-update` reach stakeholders, so they default to a draft for the PM to review — direct send only on explicit instruction (and Gmail can never send autonomously at all, by connector design).
